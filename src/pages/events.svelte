<script lang="ts">
  ...
  let displayed = $derived(activeFilter === 'upcoming' ? upcoming : past);
  ...
  function isExternal(event: TesseraEvent): boolean {
    return !event.speaker && !event.talk && !event.description && !event.image && !!event.externalUrl;
  }
</script>
        ...
        <li class="event-row">
          {#if isExternal(event)}
            ...
            >
              ...
              <svg class="arrow-icon" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
