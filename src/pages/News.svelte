        ...
        <div class="timeline-entry" class:is-link={!!post.externalUrl} class:is-minor={!!post.minor} id={post.slug}>
          ...
          <div class="timeline-content">
            {#if post.minor && post.externalUrl}
              <a href={post.externalUrl} target="_blank" rel="noopener" class="minor-link"><span class="minor-title">{post.title}</span><span class="minor-source">{post.sourceLabel ?? post.author ?? domain(post.externalUrl)}</span><svg class="external-icon" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3.5 1.5h7v7M10 2L4 8"/></svg></a>
